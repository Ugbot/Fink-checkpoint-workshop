...
/**
 * Scenario 02 — App2 (EXACTLY_ONCE, reads read_committed)
 *
 * CLI / env vars:
...
public class App2Pipeline {
    ...
    public static void main(String[] args) throws Exception {
        ...
        LOG.info("Starting Scenario 02 App2 — EXACTLY_ONCE, reads read_committed, kafka={}",
                 params.kafka());
        ...
        CheckpointConfigurator.configure(
            ...
            params.checkpointDir("s02-app2")
        );

        env.fromSource(
                KafkaSourceFactory.readCommitted(params.kafka(), Constants.TOPIC_MID, "s02-app2"),
            ...
            .name("10s tumbling count per account")
            .sinkTo(KafkaSinkFactory.exactlyOnce(params.kafka(), Constants.TOPIC_OUT, "s02-app2"))
            .name("KafkaSink (EXACTLY_ONCE) -> " + Constants.TOPIC_OUT);

        env.execute("S02-App2-ExactlyOnce");
    }
